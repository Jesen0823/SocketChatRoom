...
<img src="doc/网络运营商与NAT.jpg" alt="网络运营商与NAT" style="zoom:50%;" />

#### 心跳包的作用

1. 客户端与服务器之间相互告知状态
2. 通过定时发送信息，让中间运营商的NAT路由表得以维持;避免路由表过期带来的异常中断
3. 由于其约定性、定时性所以称为心跳包

#### 心跳策略选择

1. TCP KeepAlive 7200 2H ACKTCP ，但是 KeepAlive仅代表连接保持，而不代表对方业务层是否可消费数据。

2. 发送紧急数据包：

   ```java
   socket.sendUrgentData(0xff); // 一个字节，取int低8位
   ```

   但是与业务层数据比较容易冲突。

3. 发送心跳包帧，发送无数据的数据帧，定义特殊标识。

​       <img src="doc/心跳数据包结构.jpg" alt="心跳数据包结构" style="zoom:50%;" />

#### 心跳包发送与消费

1. 客户端发送心跳数据，服务器接收数据，但并不回送业务数据包;此时客户端依然会收到数据确认包(ACK)

2. 服务器使用客户端发送心跳包时长的5-10倍时长扫描客户端方案，进行客户端活跃性扫描，超出时间未活跃则自动关闭链接。
3. 服务器定时器扫描客户端状态。

...
<img src="capture\分片数据传输过程.jpg" style="zoom:50%;" />
...
另一个引起连接异常断开的是网络运营商的NAT路由器，**路由器的映射状态表**会过期，从**而引起连接断开**。
...
当连接数达到百十万级别，会创建失败，是因为系统单进程“文件句柄”数量最大限制。

实际上服务器是一群服务器设备集群，多个子端口，对外提供一个端口。
